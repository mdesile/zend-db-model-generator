...
class Make_pgsql extends MakeDbTable {
    ...
    protected function getPDOString($host, $port = 5432, $dbname) {
    ...
    public function parseForeignKeys() {
        ...
        if (!$query) {
            throw new Exception("{$sql} returned false!.");
        }

        if (count($res = $query->fetchAll()) < 0) {
            throw new Exception("{$sql} did not provide known output");
        }
        ...
        foreach ($res as $row) {
            ...
            $keys[$key] = array(
                ...
                'foreign_tbl_name' => $row ['foreign_table_name'],
        ...
        //var_dump(array('foreign_keys', $keys));
    ...
    public function parseDependentTables() {
        ...
        foreach ($tables as $table) {
            $sql = "SELECT tc.constraint_name, tc.table_name, tc.constraint_type,
        ...
        //var_dump(array('dependencies', $dependents));
        ...
        foreach ($query->fetchAll() as $column) {
            ...
            if ($column ['constraint_type'] == 'PRIMARY KEY') {
                $primaryKey [] = array(
                    ...
                    'var_capital' => $this->_getVarCapital($column['column_name']),
